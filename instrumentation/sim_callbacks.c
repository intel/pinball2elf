...
void elfie_on_start(uint64_t num_threads, void* context)
{
  ...
  lte_syscall(__NR_write, 2, (uint64_t)str, sizeof(str)-1, 0, 0, 0); 
  retval = lte_syscall(__NR_mlockall, MCL_CURRENT | MCL_FUTURE, 0, 0, 0, 0, 0);
  lte_syscall(__NR_fsync, 2, 0, 0, 0, 0, 0);
  if (retval !=0 )
   lte_syscall(__NR_write, 2, (uint64_t)mlockerrmesg, sizeof(mlockerrmesg)-1, 0, 0, 0); 
  lte_syscall(__NR_fsync, 2, 0, 0, 0, 0, 0);
