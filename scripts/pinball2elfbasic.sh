...
#0,370148,370148,8,SYS_lseek,1,0,0,,.
      ...
      ;;
      "SYS_chdir")
#0,6293565,6293565,80,SYS_chdir,0,0,0,/media/disk2/.../ned
...
if  [ $# -eq 3 ];  then
    ...
    if [ ! -z $strace ];
    then
        cat $INST/basic_callbacks.c | awk '/void elfie_on_start/ {print "#define MYBUFSIZE 20480000\n__lte_static char mybuf[MYBUFSIZE];\n__lte_static char s_readerror[] = \" <-- actual bufsize. MYBUFSIZE too small for SYS_read \\n\";\n__lte_static int do_replay_syscalls = 1;\nvoid replay_syscalls();"}
        {print $0}'> $TMPDIR/basic_callbacks.$$.c
    else
        cat $INST/basic_callbacks.c | awk '/void elfie_on_start/ {print "__lte_static char mybuf[1];\n__lte_static int do_replay_syscalls = 0;\nvoid replay_syscalls();\n"}
        {print $0}'> $TMPDIR/basic_callbacks.$$.c
    fi
    if [ ! -z $strace ];
    then
        STRACE $strace $psgi $TMPDIR/basic_callbacks.$$.c
    else
      echo "void replay_syscalls(){}" >> $TMPDIR/basic_callbacks.$$.c
    fi
    gcc -g -I$PINBALL2ELFLOC/lib -c $TMPDIR/basic_callbacks.$$.c -o $TMPDIR/basic_callbacks.$$.o
    time  $PINBALL2ELF --text-seg-flags WXA --data-seg-flags WXA --cbk-stack-size 102400 --modify-ldt -u unlimited --roi-start ssc:$sscmark  --roi-start simics:$magicval --magic2 simics:$magicval2 -l 0x0 -i 0 -d $tmpBASE.global.log -m $tmpBASE.text -r $tmpBASE.address -x $BASE.elfie -p elfie_on_start -t elfie_on_thread_start $TMPDIR/basic_callbacks.$$.o  $PINBALL2ELFLOC/lib/libperfle.a   $PINBALL2ELFLOC/lib/libcle.a  
