...
if  [ $# -lt 2 ];  then
    ...
    magicval2=0x2
    sscmark=0x111
    cp $INST//basic_callbacks.c /tmp/basic_callbacks.$$.c
    if [ ! -z $sysstate ];
    then
      PREOPEN $sysstate /tmp/basic_callbacks.$$.c
    else
      echo "WARNING: No sysstate directory exists for $BNAME"
      echo "void preopen_files(){}" >>  /tmp/basic_callbacks.$$.c
    fi
    if [ ! -z $sysstate ];
    then
      ...
      then
        ...
        SETHEAP $BRKfile /tmp/basic_callbacks.$$.c
      else
        echo "void set_heap(){}" >>  /tmp/basic_callbacks.$$.c
      fi
    else
        echo "void set_heap(){}" >>  /tmp/basic_callbacks.$$.c
    fi
    gcc -g -I$PINBALL2ELFLOC/lib -c /tmp/basic_callbacks.$$.c -o /tmp/basic_callbacks.$$.o
    time  $PINBALL2ELF --text-seg-flags WXA --data-seg-flags WXA --cbk-stack-size 102400 --modify-ldt -u unlimited --roi-start ssc:$sscmark  --roi-start simics:$magicval --magic2 simics:$magicval2 -l 0x0  $IINFO -i 0 -d $tmpBASE.global.log -m $tmpBASE.text -r $tmpBASE.address -x $DEST -p elfie_on_start -t elfie_on_thread_start /tmp/basic_callbacks.$$.o  $PINBALL2ELFLOC/lib/libperfle.a   $PINBALL2ELFLOC/lib/libcle.a  
