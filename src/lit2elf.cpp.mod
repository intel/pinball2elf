...
static lte_addr_t litelfMarkDynallocPages(lte_memimg_t& memimg, lte_x86_arch_state_t& arch_state, elf_table_t& dynpages)
{
   ...
   if(!get_config().no_stack_remap())
   {
      ...
      if(get_config().is_verbose())
      {
         if(pages.size())
         {
            const char* fmt = (arch_state.get_arch() == ELFCLASS64) ? " %016" PRIx64 " - %016" PRIx64 "\n"
                                                                    : " %08"  PRIx64 " - %08"  PRIx64 "\n";
            fprintf(stdout, "\n---------------- STACK --------------\n");
            for(std::map<lte_addr_t, lte_mempage_t*>::iterator it = pages.begin(); it != pages.end(); ++it)
            {
               lte_uint64_t va = it->second->va;
               fprintf(stdout, fmt, va, va + memimg.page_size);
            }
            fprintf(stdout, "-------------------------------------\n\n");
         }
         else
   ...
   if(remap_limit < (LTE_MAXVAL(lte_addr_t)-memimg.page_size))
   {
      ...
      {
        if (it->first == 0x400000)
         {
            std::cout<< "Found 0x400000\n";
            //it->second.flags |= SHF_DYNALLOC;
            it->second.flags |=  SHF_ALLOC | SHF_EXECINSTR;
            //pages[it->first] = &it->second;
         }
         else if(((it->first + memimg.page_size) > remap_limit) && (pages.count(it->first) == 0))
         {
            it->second.flags |= SHF_DYNALLOC;
            pages[it->first] = &it->second;
         }
...
int main(int argc, char** argv)
{
   ...
   */
   ...
   litelfCreateSymtabSection(*elf, *symtab)->set_sh_link(litelfCreateStrtabSection(*elf, strtab)->get_index());
   ...
   if(relatab->get_entries_num())
   {
      ...
      rela_text->set_data(relatab);
      rela_text->set_sh_entsize(relatab->get_entry_size());
      rela_text->set_sh_link(elf->get_section(SymtabSegName)->get_index());
      rela_text->set_sh_info(elf->get_section(TextSegName)->get_index());
